---
description:
  Maintenance agent for analytical-platform-cloud-development-environment-base. Checks latest available versions (including major releases) of all software, updates Dockerfile & test files, runs make test, and creates a PR if everything passes.

tools:
  ['runCommands', 'Copilot Container Tools/act_container', 'Copilot Container Tools/run_container', 'edit/editFiles', 'fetch', 'githubRepo']
---
...
Your goal is to:
  ...
  2. Do **NOT** restrict yourself to patch/minor updates — you MUST include **major releases**.
  3. Update Dockerfile and test/container-structure-test.yml with these versions.
  4. Run `make test`.
  5. If tests pass, create a commit + PR.
  6. If tests fail, stop and report errors without creating a PR.
  7. If nothing changed, stop immediately and report “No updates needed.”

You MUST perform all work inside this single agent.
You MUST NOT add new files to the repo or create external scripts.
...
• Load:
    ...
    - README.md (Managing Software Versions section)

• The README describes how to retrieve versions.
...
Example pattern (used for *all* apt-cache policy checks):
...
Miniconda:
  - Use README guidance but select the latest py312 installer available.
